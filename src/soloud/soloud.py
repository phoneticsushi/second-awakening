...
# This file has been modified from the autogenerated file produced by SoLoud
...
for filename in ['soloud_x86', 'soloud_x64', 'libsoloud.so']:
	try:
		...
		soloud_dll = ctypes.CDLL(str(dll_path))
		break
	except FileNotFoundError:
		# on Windows, we get explicit an FileNotFoundError
		continue
	except OSError as e:
		...
		if 'No such file or directory' in str(e):
			...
			# On Windows, but DLL loading failed - loading 32-bit version on 64-bit Windows e.g.
			continue
		else:
...
if not soloud_dll:
	raise Exception(f'Compatible SoLoud dynamic library not found in {dir_path}')
...
class Monotone(object):
	...
	def quit(self):
		Monotone_destroy(self.objhandle)
		self.objhandle = ctypes.c_void_p(0)
	def set_params(self, aHardwareChannels, aWaveform = 0 ):  # 0 == SoLoud::Misc::WAVE_SQUARE
